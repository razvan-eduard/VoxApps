...
fun PipedSettingsSection(
    ...
    SettingsSectionCard(languageManager.getString("youtube_url_engine")) {
        Picklist(
            items = backends,
    ...
    // Instance and region belong to a hosted backend; the on-device one has neither.
    if (selectedBackend?.isBuiltIn == true) return

    SettingsSectionCard(languageManager.getString("piped_section")) {
