...
class ExpenseDeduplicationResultParserTest {

    @Test
    fun `parses a single group`() {
        val json = """{"groups":[{"keep":12,"duplicates":[7,9]}]}"""
        val result = ExpenseDeduplicationResultParser.parse(json)
    ...
    fun `no groups key returns empty list`() {
        assertEquals(emptyList<DuplicateGroup>(), ExpenseDeduplicationResultParser.parse("{}"))
    }

    @Test
    fun `empty groups array returns empty list`() {
        assertEquals(emptyList<DuplicateGroup>(), ExpenseDeduplicationResultParser.parse("""{"groups":[]}"""))
    }
    ...
    fun `malformed json returns null`() {
        assertNull(ExpenseDeduplicationResultParser.parse("{ not json"))
    }
}
