...
object BudgetMath {
    ...
    /** The record set a budget is a summary of: its account, its currency, its window. */
    fun applies(budget: AccountBudget, expense: Expense, windowStart: Long): Boolean =
        expense.bankAccountId == budget.accountId &&
    ...
    fun remaining(
        ...
        val movement = expenses.filter { applies(budget, it, start) }.sumOf {
    ...
    fun spent(
        ...
        expenses: List<Expense>,
        today: LocalDate = LocalDate.now()
    ): Double = openingBalance(budget) - remaining(budget, expenses, today)
}
