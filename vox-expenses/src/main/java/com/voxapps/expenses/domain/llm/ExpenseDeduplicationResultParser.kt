...
/**
 * Parses the expense-deduplication LLM response
 * `{"groups": [{"keep": <id>, "duplicates": [<id>,...]}]}` into a list of [DuplicateGroup]. Mirrors
 * vox-notes' NoteDeduplicationResultParser — tolerant of individual malformed groups, only returns
 * null if the top-level JSON itself can't be read at all.
 */
object ExpenseDeduplicationResultParser {
