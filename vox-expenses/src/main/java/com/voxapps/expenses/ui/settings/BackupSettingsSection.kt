...
import com.voxapps.expenses.receiver.ExpensesExportImportHandler
...
/** Expenses' own "back up to a file I pick right now" / "restore from a file I pick" section, built
 *  on the shared `:core:backup` card — the only app with two independent attachment zip sources
...
fun BackupSettingsSection(settingsRepo: ExpensesSettingsRepository, settings: ExpensesSettings) {
    ...
    val createDocumentLauncher = rememberLauncherForActivityResult(
        ...
        isBusy = true
        scope.launch {
            val exportResult = handler.export(exportScope, settings.backupIncludeApiKeys, settings.backupIncludeAttachments)
            val attachmentEntries = buildMap {
                exportResult.attachmentUri?.let { put("expenses-receipts.zip", Uri.parse(it)) }
                exportResult.secondaryAttachmentUri?.let { put("$DOMAIN-attachments.zip", Uri.parse(it)) }
            }
    ...
    val openDocumentLauncher = rememberLauncherForActivityResult(
        ...
        scope.launch {
            ...
            if (payload == null) {
                resultMessage = "That file has no Expenses backup in it"
                isBusy = false
                return@launch
            }
            payload.attachmentUris["receiptsZipUri"]?.let { payload.domainJson.put("receiptsZipUri", it.toString()) }
