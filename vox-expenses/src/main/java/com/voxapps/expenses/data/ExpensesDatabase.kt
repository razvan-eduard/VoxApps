...
abstract class ExpensesDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** The index the main list's shape wants: archivedAt IS NULL, ordered by dateTime — one
         *  ordered walk instead of a scan-and-sort, and the same walk a paging window steps. */
        private val MIGRATION_41_42 = object : Migration(41, 42) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE INDEX IF NOT EXISTS index_expenses_archivedAt_dateTime " +
                        "ON expenses (archivedAt, dateTime)"
                )
            }
        }

        /**
         * Record provenance and provenance-aware budgets — see [Expense.originDeviceId] and
         * [SpendingLimit.ownDeviceOnly]. Existing rows backfill to null origin ("made here"), which
         * is true of every row written before a sync merge could insert one; existing limits
         * backfill to own-device-only, keeping the totals they watched unchanged.
         */
        private val MIGRATION_42_43 = object : Migration(42, 43) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE expenses ADD COLUMN originDeviceId TEXT")
                db.execSQL("ALTER TABLE expenses ADD COLUMN originDeviceName TEXT")
                db.execSQL("ALTER TABLE spending_limits ADD COLUMN ownDeviceOnly INTEGER NOT NULL DEFAULT 1")
            }
        }
        ...
        private val MIGRATION_37_38 = object : Migration(37, 38) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE expenses ADD COLUMN archivedAt INTEGER")
            }
        }

        /** Where a record's fields came from — see [Expense.originsJson]. */
        private val MIGRATION_36_37 = object : Migration(36, 37) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE expenses ADD COLUMN originsJson TEXT")
            }
        }

        /** A card that has been replaced is kept and stopped being offered — see
         *  [BankAccount.archived]. */
        private val MIGRATION_35_36 = object : Migration(35, 36) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE bank_accounts ADD COLUMN archived INTEGER NOT NULL DEFAULT 0")
            }
        }

        /** What there is to spend on an account, per currency it holds — see [AccountBudget]. */
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS account_budgets (" +
                        ...
                        "reconciledRemaining REAL)"
                )
                db.execSQL(
                    "CREATE UNIQUE INDEX IF NOT EXISTS index_account_budgets_accountId_currencyCode " +
                        "ON account_budgets(accountId, currencyCode)"
                )
            }
        }
