 ...
 * suggestion for review (not applying it), so this worker's job is done as soon as the request is
 * durably enqueued — see [CategoryAutoMergeWorker]'s doc comment on why no Commander-installed
 * pre-check is needed anymore.
 */
class NoteDeduplicationWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        NoteDeduplicationRequestSender.send(applicationContext, container.pendingLlmRequestQueue, notes)
