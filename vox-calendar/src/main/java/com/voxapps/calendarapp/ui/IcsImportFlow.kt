...
/**
 * The whole import journey behind one call: pick an .ics file, parse it, ask which calendar it
 * lands in — a new one, named and colored, or an existing local one — write the entries, and say
 * how many. Returns the launch lambda; the target dialog rides along in this composition. Both
 * doors to importing (the sidebar's + menu and the ICS settings tab) call this one flow.
 */
@Composable
fun rememberIcsImportFlow(calendarRepository: CalendarRepository): () -> Unit {
    ...
    return { importLauncher.launch(arrayOf("text/calendar", "*/*")) }
}
