...
# Regenerates the "Latest Releases" table in README.md (between the
# LATEST_RELEASES:START/END markers) from the repo's actual GitHub Releases — never hand-edit tag
# numbers there, they go stale the moment any app's version is bumped. Run manually
# (`./scripts/update_release_readme_links.sh`) or via `.github/workflows/update-readme-releases.yml`,
# which runs this on every `release: published` event so the table can't drift from what's actually
# published. Requires `gh` (authenticated) and `jq`.
...
ALL_RELEASES_JSON=$(gh api "repos/$REPO/releases" --paginate --jq '[.[] | select(.draft==false) | {tag: .tag_name, published_at: .published_at, prerelease: .prerelease}]')
...
while IFS=':' read -r prefix display asset_prefix; do
    ...
    ASSET_URL="https://github.com/$REPO/releases/download/$TAG/${asset_prefix}-${TAG}.apk"
    ...
    TABLE_ROWS="${TABLE_ROWS}| **${display}** | [\`${LABEL}\`](${RELEASE_URL}) | [Download APK](${ASSET_URL}) |
"
done <<< "$APPS"
...
log_info "✅ README.md's Latest Releases table updated."
