...
The top-level `README.md` has one "Build Status" table (one row per app: build badge, latest tag, APK
size, direct download link) regenerated by `scripts/update_release_readme_links.sh` — it queries this
repo's actual GitHub Releases via `gh api`, groups them by tag prefix, picks the newest per app, reads
each release's real published asset size directly from the API response (not a hand-maintained
