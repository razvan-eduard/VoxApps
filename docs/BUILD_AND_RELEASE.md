...
`vox-commander`'s `preBuild` compiles whisper.cpp from its submodule and asks JitPack whether Vosk,
NewPipeExtractor and OpenWakeWord have moved. That is what a developer machine and a release runner
want, and not what a quick compile check wants:

```
./gradlew :vox-commander:assembleDebug -PvoxSkipNativePrep
```

skips all four. Everything downstream of `preBuild` still runs, so a change that breaks Commander's
Kotlin still fails. `copyShippedSchemas` is **not** skippable — the schema tests read the assets it
generates.
...
`vendor/opencv` commit plus `scripts/build_opencv_android.sh`; on a hit, the submodule fetch, the
Android SDK setup and the NDK install are all skipped. OpenCV cannot be skipped the way whisper can
— `vendor/ppocr-sdk/opencv/java` is generated by that build and `org.opencv.*` comes from it.
...
Six `sync-*.yml` workflows watch vendored native upstreams — `vosk`, `newpipe-extractor`,
`openwakeword`, `opencv`, `ppocr-sdk` weekly (Mondays 06:00 UTC) and `whisper` monthly (the 1st).
They apply the update, try to build it, and open a PR saying whether it compiled. See
`BUILD_TIME_DEPENDENCIES.md` for what each vendored fork patches and how those patches are kept.
