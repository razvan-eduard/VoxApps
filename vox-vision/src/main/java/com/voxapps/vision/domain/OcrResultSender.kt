...
object OcrResultSender {
    fun send(context: Context, sourcePackage: String, result: VoxOcrResult) {
        ...
        // field or ClipData — they don't peek into extras strings. We duplicate each URI in ClipData
        // purely to satisfy the permission system; OcrResultReceiver still reads the full payload.
        // A multi-shot reply carries several images (see VoxOcrResult.imageUris), so this grants and
        // adds one ClipData item per URI instead of assuming exactly one.
        result.imageUris.forEachIndexed { index, uriString ->
            try {
                ...
                if (intent.clipData == null) {
                    intent.clipData = android.content.ClipData.newRawUri("receipt_image_$index", uri)
                } else {
                    intent.clipData!!.addItem(android.content.ClipData.Item(uri))
                }
            } catch (e: Exception) {
